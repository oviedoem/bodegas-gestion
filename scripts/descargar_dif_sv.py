...
SELECT TOP 50
    ...
    ISNULL(ENC.ESTACION, '')                       AS PC,
    ISNULL(N.IDMEDIDA, '')                         AS UNIDAD,
    CAST(ISNULL(N.VALOR_LISTA, 0) AS DECIMAL(18,2))          AS VALOR_LISTA,
    CAST(ISNULL(N.DESCTO1, 0) AS DECIMAL(18,4))              AS DESC_PCT,
    CASE WHEN ISNULL(N.CANTIDAD,0)<>0
         THEN CAST(ISNULL(N.VALOR_NETO,0)/N.CANTIDAD AS DECIMAL(18,2))
         ELSE 0 END                                AS VALOR_NETO_UNI,
    ISNULL(ENC.IDENTIDAD, '')                      AS IDCLIENTE,
